...
for (const variantId of [variantIds[2], variantIds[3], variantIds[4], variantIds[5]]) {
  const trace = expectSolvedTrace(variantId);
  assert.ok(
    trace.events.some(
...
for (const variantId of [variantIds[4], variantIds[5]]) {
  const trace = expectSolvedTrace(variantId);
  assert.ok(
    trace.events.some(
...
for (const variantId of variantIds) {
  ...
  if (variantId === variantIds[5]) {
    assert.ok(eventTypes(trace).has("clique-prune"), `${variantId} should report the clique lower-bound prune`);
  } else {
    assert.ok(!eventTypes(trace).has("clique-prune"), `${variantId} should not use clique lower-bound pruning`);
  }
}
