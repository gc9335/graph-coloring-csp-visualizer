...
COMPAT_FIELDNAMES = [field for field in FIELDNAMES if field not in {"canonical_solutions_found", "signature_solutions_found"}]
LEGACY_FIELDNAMES = [
    field
...
def values_to_row(values: list[str]) -> dict[str, object]:
    ...
    if len(values) == len(COMPAT_FIELDNAMES):
        compat_row = {field: values[index] for index, field in enumerate(COMPAT_FIELDNAMES)}
        expanded: dict[str, object] = {}
        for field in FIELDNAMES:
            if field in {"canonical_solutions_found", "signature_solutions_found"}:
                expanded[field] = compat_row.get("solutions_found", "")
            else:
                expanded[field] = compat_row.get(field, "")
        return expanded
    if len(values) == len(LEGACY_FIELDNAMES):
        ...
        for field in FIELDNAMES:
            if field == "target_density":
                ...
                expanded[field] = legacy_row.get("solutions_found", "")
            else:
...
def make_result_row(job: ExperimentJob, variant_id: str, variant_name: str, result) -> dict[str, object]:
    ...
    return {
        ...
        "canonical_solutions_found": result.canonical_solutions_found,
        "signature_solutions_found": result.signature_solutions_found,
