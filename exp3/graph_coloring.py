...
class SolveResult:
    ...
    solutions_found: int
    canonical_solutions_found: int
    signature_solutions_found: int
    solution_target: int
...
def canonicalize_coloring(colors: list[int]) -> tuple[int, ...]:
    remap: dict[int, int] = {}
    next_color = 1
    canonical: list[int] = []
    for color in colors:
        ...
        if color not in remap:
            ...
            next_color += 1
        canonical.append(remap[color])
    return tuple(canonical)


def coloring_signature(colors: list[int]) -> tuple[tuple[int, ...], ...]:
    color_classes: dict[int, list[int]] = {}
    for node, color in enumerate(colors):
        if color == 0:
            continue
        color_classes.setdefault(color, []).append(node)
    return tuple(sorted(tuple(nodes) for nodes in color_classes.values()))
...
def load_dimacs_graph(filepath: str | Path) -> GraphData:
    ...
    if path.name in PREFERRED_START_NODES:
        metadata["preferred_start_node"] = PREFERRED_START_NODES[path.name]
...
class GraphColoringSolver:
    def __init__(self, adj_list: list[list[int]], num_colors: int, preferred_start_node: int | None = None):
        ...
        self.preferred_start_node = preferred_start_node if preferred_start_node in range(self.n) else None
        if self.preferred_start_node is not None:
            self.static_order = [self.preferred_start_node] + [
                node for node in self.static_order if node != self.preferred_start_node
            ]
        ...
        self._canonical_solutions_seen: set[tuple[int, ...]] = set()
        self._signature_solutions_seen: set[tuple[tuple[int, ...], ...]] = set()
    ...
    def solve(
        ...
        *,
        ...
        if self.preferred_start_node is not None:
            explanation_notes.append(f"preferred_start_node={self.preferred_start_node + 1}")
        ...
        if variant.component_decomposition and len(components) > 1:
            ...
            for component in components:
                ...
                if not sub_result.success:
                    ...
                    return SolveResult(
                        ...
                        canonical_solutions_found=total_solutions,
                        signature_solutions_found=total_solutions,
                        solution_target=max(1, max_solutions),
                        stopped_on_target=False,
                        component_count=len(components),
                        largest_component=largest_component,
            ...
            return SolveResult(
                ...
                canonical_solutions_found=max(1, total_solutions),
                signature_solutions_found=max(1, total_solutions),
                solution_target=max(1, max_solutions),
        ...
        self._canonical_solutions_seen = set()
        self._signature_solutions_seen = set()
        ...
        return SolveResult(
            ...
            canonical_solutions_found=len(self._canonical_solutions_seen),
            signature_solutions_found=len(self._signature_solutions_seen),
    ...
    def _search(self, variant: Variant, depth: int) -> bool:
        ...
        if node is None:
            ...
            self._canonical_solutions_seen.add(canonicalize_coloring(self.colors))
            self._signature_solutions_seen.add(coloring_signature(self.colors))
    ...
    def _choose_node(self, variant: Variant) -> int | None:
        ...
        if not unassigned:
            return None
        if self.preferred_start_node is not None and self.colors[self.preferred_start_node] == 0 and all(
            color == 0 for color in self.colors
        ):
            return self.preferred_start_node
        if not variant.mrv:
            return unassigned[0]
...
def run_variant_on_graph(
    ...
    preferred_start_node = graph.metadata.get("preferred_start_node")
    solver = GraphColoringSolver(
        graph.adj_list,
        color_limit,
        preferred_start_node=int(preferred_start_node) if preferred_start_node is not None else None,
    )
    return solver.solve(
        variant,
